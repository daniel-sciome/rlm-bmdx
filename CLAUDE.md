...
- [ ] **Unify file upload stores into a single pool** — Three parallel in-memory dicts (`_bm2_uploads` in server_state.py, `_csv_uploads` in server_state.py, `_data_uploads` in pool_orchestrator.py) each with different schemas, checked in sequence by the preview endpoint, and re-registered separately during session restore. Consolidate into one `_file_pool: dict[str, FileEntry]` with a shared schema (id, filename, path, file_type, parsed_data). Simplifies preview (one lookup), session restore (one registration path), and eliminates the "which dict does this file live in?" question that makes the code hard to trace.
