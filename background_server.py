...
# Shared state dicts live in pool_orchestrator.py — we grab references here
# so existing code can use the same names without a full rewrite.
...
# _js_dose_key and _serialize_table_rows live in pool_orchestrator.py.
# Local aliases for call sites that haven't been migrated yet.
_js_dose_key = pool_orchestrator._js_dose_key
...
_ensure_fingerprints = ensure_fingerprints
_fingerprint_and_store = fingerprint_and_store
_run_lightweight_validation = run_lightweight_validation
...
# ---------------------------------------------------------------------------
# File pool orchestration (fingerprinting, validation, integration,
